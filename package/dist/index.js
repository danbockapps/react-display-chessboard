...
if (process.env.NODE_ENV !== "production") {
  (function () {
    ...
    var React = require$$0__default["default"]; // ATTENTION
...
const ChessboardContext = /*#__PURE__*/require$$0__default["default"].createContext();
const useChessboard = () => require$$0.useContext(ChessboardContext);
const ChessboardProvider = /*#__PURE__*/require$$0.forwardRef(({
  animationDuration,
  areArrowsAllowed,
  boardOrientation,
  boardWidth,
  ...
  const [currentPosition, setCurrentPosition] = require$$0.useState(convertPositionToObject(position)); // calculated differences between current and incoming positions

  const [positionDifferences, setPositionDifferences] = require$$0.useState({}); // colour of last piece moved to determine if premoving

  const [lastPieceColour, setLastPieceColour] = require$$0.useState(undefined); // current right mouse down square

  const [currentRightClickDown, setCurrentRightClickDown] = require$$0.useState(); // current arrows

  const [arrows, setArrows] = require$$0.useState([]); // chess pieces/styling

  const [chessPieces, setChessPieces] = require$$0.useState({ ...defaultPieces,
    ...customPieces
  }); // the most recent timeout whilst waiting for animation to complete

  const [previousTimeout, setPreviousTimeout] = require$$0.useState(undefined); // if currently waiting for an animation to finish

  const [waitingForAnimation, setWaitingForAnimation] = require$$0.useState(false); // handle custom pieces change
...
function Piece({
  ...
  const [pieceStyle, setPieceStyle] = require$$0.useState({
    opacity: 1,
    zIndex: 5,
    touchAction: 'none'
...
function Square({
  ...
  const squareRef = require$$0.useRef();
  const {
    boardWidth,
    boardOrientation,
    clearArrows,
...
function Board() {
  const boardRef = require$$0.useRef();
  const [squares, setSquares] = require$$0.useState({});
  const {
    arrows,
    boardOrientation,
    boardWidth,
...
const Chessboard = /*#__PURE__*/require$$0.forwardRef((props, ref) => {
